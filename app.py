...
# IMPORT DECORATOR BẮT BUỘC ĐĂNG NHẬP
...
def create_app():
    ...
    # --- ROUTE TRANG CHỦ (DASHBOARD) - BẮT BUỘC ĐĂNG NHẬP ---
    @app.route('/')
    @login_required
    def dashboard():
        ...
        for s in schedules:
            if s.date:
                ...
                t_name = "Công việc"
                if s.task:
                    t_name = getattr(s.task, 'task_name', None) or getattr(s.task, 'name', None) or "Công việc"
    ...
    # --- ROUTE BÁO CÁO - BẮT BUỘC ĐĂNG NHẬP ---
    @app.route('/report')
    @login_required
    def report():
        total_emp = Employee.query.count()
    ...
    # --- ROUTE CÀI ĐẶT - BẮT BUỘC ĐĂNG NHẬP ---
    @app.route('/settings')
    @login_required
    def settings():
